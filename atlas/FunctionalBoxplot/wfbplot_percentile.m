...
function [depth, outpoint, medianCurveId]=wfbplot_percentile(data,x,weight,depth,method,show,prob,color,outliercol,barcol,fullout,factor)
...
for pp=1:length(prob)
	...
	%end	
	weight_sum = weight_sum + weight( index( iI ) );
	if weight_sum >= 0.993 && index_95_flag == 0
		index_95 = iI;
		index_95_flag = 1;

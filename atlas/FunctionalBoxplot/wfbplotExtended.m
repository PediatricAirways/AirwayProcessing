...
function [depth, index, outpoint, medianCurveId, inf, sup, mincurve, maxcurve, ranklist]= ...
    wfbplotExtended(data,x,weight,depth,method,show,prob,color,outliercol,barcol,fullout,factor,sampleData)
...
if nargin<3,    weight=ones(size(data, 2), 1) / size(data, 2); end
if nargin<2,    x=[]; end
...
for pp=1:length(prob)
    ...
    %region_95 = data(:, index(1:index_95));
    %out = data(:, index((index_95+1):n));
    %inf_95 = min(region_95, [], 2)';
    %sup_95 = max(region_95, [], 2)';
       %{ 
        if show 
			hold all;
	...
	if sum(outly)>0
		if show 
			...
			for iOut = 1:length(weight_out)
				plot( x, out(:, iOut), 'Color', [1.0-weight_out(iOut)*length(weight_out)/4 ...
                    1.0-weight_out(iOut)*length(weight_out)/4 1.0-weight_out(iOut)*length(weight_out)/4], 'LineStyle', '--' );
			end
...
%  		end
    ...
    % order the curves based on the depth
    
    
    ranklist = order_curves( data, index, outpoint );
    %ranklist = [];
    
    %{ 
    % failed
    ranklist = zeros( 1, length(index));
    for iI = 1:length(index)
        ranklist(iI) = index(iI);
        if iI <= 1 continue; end
        curve_lower = min( data( :, index(1:iI) ), [], 2 );
        curve_upper = max( data( :, index(1:iI) ), [], 2 );
        dist1 = sum( and( data( :, iI ) >= curve_lower, data( :, iI ) <= data( :, index(1) ) ) );
        dist2 = sum( and( data( :, iI ) >= data( :, index(1) ), data( :, iI ) <= curve_upper ) );
        if dist1 >= dist2
            ranklist(2:iI) = ranklist(1:iI-1);
            ranklist(1) = index(iI);
        end
    end
    %}
    ...
    %{
    dataTmp = [ data(:, 1:index(1)-1), data(:, index(1)+1:end)];
    for iI = 1:size(dataTmp, 2)
        dataTmp(:, iI) = dataTmp(:, iI) - data(:, index(1) );
    end
    IDX = kmeans( dataTmp', 2 );
    ranklist = zeros( 1, length(index) );
    ranklist(1) = index(1);
    figure, hold on
    for iI = 2:length(index)
        ranklist(iI) = index(iI);
        %dataNew = reshape( dataTmp(:, iI-1), size(dataTmp, 1)/2, 2 );
        if IDX(iI-1) == 1
            plot( dataTmp(:, iI-1), 'b-.' );
        else
            plot( dataTmp(:, iI-1), 'r--' );
